    ...
    \begin{itemize}
      ...
      \item BMI: Body Mass Index, valore dicotomizzato.
        \begin{enumerate}
          ...
          \item BMI $\ge 30$ 
        \end{enumerate}
      \item Fitness: statico fisico dell'indiviudo.
        \begin{enumerate}
          \item Much Worse 
          \item Little Worse
          \item Just as good
...
\section{Regressioni Logistiche Semplici}
  ...
  \subsection{Age}
    ...
    \begin{itemize}
      ...
      \item La variabile Age è significativa secondo il \emph{p-value}.
    \end{itemize}
  ...
  \subsection{Sex}
    ...
    \begin{itemize}
      \item Nella regressione logistica semplice, il sesso Maschile 
            ...
            rispetto al sesso Femminile, con valore stimato: SexMale $\sim$ 0.910.
      \item La variabile Sex risulta significativa secondo il \emph{p-value}, 
  ...
  \subsection{BMI}
    ...
    Le due curve sono approssivamente simili, con un leggero aumento per coloro
  ...
  \subsection{Fitness}
    ...
    se di poco, positiva per l'insorgenza di un problema cardiovascolare. \par
...
\section{Considerazioni sul Modello}
  ...
  \subsection{Maschio e Femmina} 
    ...
    modelli, abbiamo notato come ci siano stato sempre diverse probabilità 
    ...
    Possiamo notare ancora come statisticamente il sesso maschile rimanga il 
    soggetto che ha più rischi di CVD rispetto al genere femminile.
  ...
  \subsection{Attività Fisica}
    ...
    Come è possibile notare, anche nella visualizzazione del grafico, non
    sembrano mostrarci differenze tra le due categorie di PA.
...
\section{Interazioni fra le variabili}
  ...
  A differenza di Smoke e Alchol, l'interazione tra Smoke e BMI sembra 
  mostrare un'interazione significativa, variando il valore atteso e il 
  significato della variabile BMI. \par
  In questo caso la variabile BMI assume valore negativo, influendo negativamente
  nella comparsa di CVD.
  
  \subsection{Alchol e BMI}
  Come per il caso di Smoke, verifichiamo se il consumo di alchol associato ad un
  maggior indice di massa corporea influisce nella probabilità di CVD.
  ...
  \subsection{Commento}
    ...
    un problema di overfitting, dove il modello cerca di avvicinarsi di più hai 
    ...
    il grado, non fornire un contributo decisivo per il nostro problema,
  ...
  @
   ...
   \subsection{Backward}
    ...
    \subsection{Commento}
      ...
      che si pone in mezzo alle due procedure AIC e BIC. \par
...
\section{Test sul Modello}
  ...
  \subsection{Sex}
    ...
    <<Dato_UomoDonna>>=
    #Dato Uomo
    #Intercetta: 1, sexMale: 1, Age: 55, BMI: 0, Fitness: 5, 
    #Smoke:Former: 1, Smoke:NO: 0 
    t.sex.man <- c(1, 1, 55, 0, 5, 1, 0)
    
    #Dato Donna
    #Intercetta: 1, sexMale: 0, Age: 55, BMI: 0, Fitness: 5, 
    #Smoke:Former: 1, Smoke:NO: 0 
    t.sex.woman <- c(1, 0, 55, 0, 5, 1, 0)
    @
    ...
    \begin{itemize}
      \item Probabilità per l'uomo: $\hat{\pi}$ = 0.048
      \item Probabilità per la donna: $\hat{\pi}$ = 0.023
    \end{itemize}
...
\section{Conclusioni}
  ...
  Modello: $\sim$ CVD $\sim$ Sex + Age + BMI + Fitness + Smoke. \par
  ...
  categorie e contemporaneamente risulta essere un modello abbastanza semplice e      significativo.
